...
export default function RFQSupplierHub() {
  ...
  if (!rfq) return <div className="p-8 text-center text-slate-500">RFQ not found</div>;
  ...
  return (
    <div className="max-w-5xl mx-auto py-8 px-4">
      <div className="flex items-center gap-4 mb-8">
        <Link href="/rfqs" className="p-2 bg-white rounded-full border border-slate-200 hover:bg-slate-50 text-slate-600 shadow-sm">
          <ArrowLeft size={18} />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-slate-800">RFQ Supplier Hub</h1>
          <div className="text-sm text-slate-500 font-mono mt-1">{rfq.rfqNumber} • Status: <strong className="text-slate-700">{rfq.status.replace(/_/g, ' ')}</strong></div>
        </div>

        <div className="ml-auto flex gap-2">
          {rfq.status === "DRAFT" && quotations.length > 0 && (
            <button onClick={() => updateStatus("ISSUED")} className="btn-primary flex items-center gap-2">
              <Send size={16} /> Mark as Issued
            </button>
          )}
          {rfq.status === "ISSUED" && (
            <button onClick={() => updateStatus("WAITING_FOR_SUPPLIER")} className="btn-secondary flex items-center gap-2">
              <Clock size={16} /> Waiting for Suppliers
            </button>
          )}
          {rfq.status === "WAITING_FOR_SUPPLIER" && (
            <button onClick={() => updateStatus("COMPLETED")} className="btn-success flex items-center gap-2">
              <FileCheck2 size={16} /> Mark Completed
            </button>
          )}
        </div>
      </div>
      ...
      {quotations.length === 0 ? (
        <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl p-12 flex flex-col items-center justify-center text-slate-500">
          ...
          <p className="text-sm mt-1">Select a supplier from the dropdown above to generate an RFQ file.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {quotations.map((q: any) => (
            <div
              key={q.id}
              onClick={() => router.push(`/rfqs/${rfq.id}/supplier/${q.id}`)}
